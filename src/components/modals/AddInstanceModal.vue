<template>
    <BaseModal
        v-model="store.isAddInstanceModalOpen"
        :title="languageStore.getTranslation('Launcher.addInstance.title')"
    >
        <div class="space-y-4">
            <!-- Instance Name -->
            <div class="space-y-2">
                ...
                <input
                    ...
                    :placeholder="
                        languageStore.getTranslation(
                            'Launcher.addInstance.namePlaceholder',
                        )
                    "
                />
            </div>
            ...
            <div class="space-y-2">
                ...
                >
                    <option value="" disabled>
                        {{
                            languageStore.getTranslation(
                                "Launcher.addInstance.selectVersion",
                            )
                        }}
                    </option>
                    <option
...
import { type MinecraftVersion, Loaders } from "../../lib/types";
...
watchEffect(async () => {
    if (store.isAddInstanceModalOpen && !versionsLoaded.value) {
...
const formData = ref<{
    name: string;
    version: string;
    loader: Loaders; // o directamente: loader: Loaders
}>({
    name: "",
    version: "",

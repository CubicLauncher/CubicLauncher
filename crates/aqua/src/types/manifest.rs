...
fn rule_set_applies(rules: &[Rule]) -> bool {
    if rules.is_empty() {
        return true;
    }
...
impl Library {
    pub fn should_include(&self) -> bool {
        match &self.rules {
            Some(rules) => rule_set_applies(rules),
            None => true,
        }
    }
...
// ─── Version manifest download artifacts ─────────────────────────────────────
...
impl VersionManifest {
    ...
    pub fn from_file(path: impl AsRef<std::path::Path>) -> Result<Self, serde_json::Error> {
        let bytes = std::fs::read(path.as_ref()).map_err(serde_json::Error::io)?;
